...
class HistorySummaries
{
    ...
    protected const AGGREGATIONS = [
        AggregationPeriod::HOURLY
            => "DATE_FORMAT(FROM_UNIXTIME(FLOOR(ts_first_event / 1000)), '%H')",
        AggregationPeriod::DAILY
            => "DATE_FORMAT(FROM_UNIXTIME(FLOOR(ts_first_event / 1000)), '%Y-%m-%d')",
        AggregationPeriod::WEEKLY
            => "DATE_FORMAT(FROM_UNIXTIME(FLOOR(ts_first_event / 1000)), '%u')",
        // Hint -> transforming sun(0)-sat(6) into mon(1)-sun(7)
        AggregationPeriod::WEEKDAY
            => "((DATE_FORMAT(FROM_UNIXTIME(FLOOR(ts_first_event / 1000)), '%w') + 6) % 7 + 1)",
        AggregationPeriod::MONTHLY
            => "DATE_FORMAT(FROM_UNIXTIME(FLOOR(ts_first_event / 1000)), '%Y%m')",
    ];
